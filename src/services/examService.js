...
import { OAB_46_SCOPE } from '@/lib/exams/oabBlueprint';
...
function examRow(data, originalPrompt = '') {
  return {
    ...
    metadata: { ...(data.metadata || {}), generationPrompt: originalPrompt, referenceScope: '46º EOU 2026 - distribuição de matérias' },
  };
}

function questionRow(examId, question) {
  return {
    exam_id: examId,
    ...
    source_metadata: { source: 'ai_generated', referenceScope: '46º EOU 2026', original: true },
  };
}
...
export async function listExams() {
  ...
  const { data, error } = await client.from('exams')
    .select('id,slug,title,exam_type,edition_number,year,source_kind,question_count,passing_score,duration_minutes,status,is_active,is_featured,updated_at')
...
export async function createExamDraft(planInput, originalPrompt) {
  ...
  if (error) throw error;
  await client.from('admin_audit_logs').insert({ action: 'create_exam_draft', entity_type: 'exam', entity_id: data.id, payload: { source: 'ai', slug } });
  return data.id;
}
...
export async function getMissingQuestionNumbers(id) {
  ...
  return OAB_46_SCOPE.map(i => i.number).filter(n => !existing.has(n));
}

export function validateGeneratedBatch(batchInput, expectedQuestions) {
  const batch = examQuestionBatchSchema.parse(batchInput);
  const expected = new Map(expectedQuestions.map(item => [item.number, item.area]));
  ...
  for (const q of batch.questions) {
    ...
    if (q.area !== expected.get(q.number)) throw new Error(`A questão ${q.number} deveria ser de ${expected.get(q.number)}, mas veio como ${q.area}.`);
...
export async function saveQuestionBatch(id, questions) {
  const client = requireClient();
  const { data: exam, error: readError } = await client.from('exams').select('status,source_kind').eq('id', id).single();
  ...
  const rows = questions.map(q => questionRow(id, q));
...
export async function updateExamDraft(id, rawInput) {
  ...
  if (parsed.questions.length) {
    const { error: qError } = await client.from('exam_questions').insert(parsed.questions.map(q => questionRow(id, q)));
    if (qError) throw qError;
  }
  await client.from('admin_audit_logs').insert({ action: 'update_exam_draft', entity_type: 'exam', entity_id: id, payload: { questionCount: parsed.questions.length } });
}
...
export async function publishExam(id) {
  ...
  if (exam.questions.length !== 80) throw new Error(`A prova possui ${exam.questions.length}/80 questões. Complete a geração antes de publicar.`);
  const expectedNumbers = OAB_46_SCOPE.map(i => i.number);
  const numbers = exam.questions.map(q => q.number).sort((a,b) => a-b);
  if (numbers.some((n,i) => n !== expectedNumbers[i])) throw new Error('A prova deve conter exatamente as questões numeradas de 1 a 80.');
  for (const q of exam.questions) {
    const expectedArea = OAB_46_SCOPE.find(i => i.number === q.number)?.area;
  ...
  if (error) throw error;
  await client.from('admin_audit_logs').insert({ action: 'publish', entity_type: 'exam', entity_id: id, payload: { questions: 80 } });
}
