import { loadProviders, newProviderId, saveProviders, toPublicProvider } from './configStore';
...
function normalizeProvider(input, existing = null) {
  ...
  const provider = {
    ...
    headersJson: typeof input.headersJson === 'string'
      ? input.headersJson
      : input.headersJson ? JSON.stringify(input.headersJson, null, 2) : existing?.headersJson || '{}',
    bodyTemplate: typeof input.bodyTemplate === 'string'
      ? input.bodyTemplate
...
async function generateOpenAICompatible(provider, requestInput) {
  ...
  const output = getPath(result.data, 'choices[0].message.content')
...
async function generateCustom(provider, requestInput) {
  ...
  if (typeof output !== 'string') {
    throw new Error(`Não foi encontrado texto no caminho de resposta: ${provider.responsePath || 'choices[0].message.content'}.`);
  }
  return { ...result, text: output.trim() };
}
...
export async function upsertProvider(input) {
  ...
  const provider = normalizeProvider(input, index >= 0 ? providers[index] : null);
